...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1): 
        ...
        # Depth Loss
        valid_depth = depth_image[:,reprojected_points[:,1].long(),reprojected_points[:,0].long()]
        depth_loss = l1_loss(valid_depth, reprojected_points[:,2])
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                print("\n[ITER {}] Evaluating {} [{} frames]: L1 {} PSNR {}".format(iteration, config['name'], len(config['cameras']), l1_test, psnr_test))
                # write to output
                with open(os.path.join(scene.model_path, "train_eval.txt"), 'a') as output_f:
...
if __name__ == "__main__":
    ...
    # https://zhuanlan.zhihu.com/p/461202163
